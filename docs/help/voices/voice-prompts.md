...
Dot.TTS works best with the exact spoken transcript. VoxCPM and Qwen3 can use cached automatic transcription when a stored transcript is unavailable, but verify pronunciation with a short test rather than assuming transcription succeeded.
...
Use search and Source, Gender, and Language filters. Edit metadata to improve filtering. Metadata labels do not alter how the recording sounds.
