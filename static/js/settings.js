...
function normalizeServerPort(value, fallback = 5000) {
...
async function refreshServerPortStatus(configuredPort) {
  const statusEl = document.getElementById("settings-server-port-status");
  const noteEl = document.getElementById("settings-server-port-note");
  if (!statusEl || !noteEl) return;

  const normalizedConfiguredPort = normalizeServerPort(configuredPort, 5000);
  ...
  try {
    ...
    if (!data.success) {
      throw new Error(data.error || "Unable to load health status");
    }

    const activePort = normalizeServerPort(
      data.server_port,
      normalizedConfiguredPort,
    );
    const preferredPort = normalizeServerPort(
      data.preferred_server_port,
      normalizedConfiguredPort,
    );
    const overriddenByEnv = !!data.server_port_overridden_by_env;
    const fallbackActive = !!data.server_port_fallback_active;

    let statusMessage = `Currently running on port ${activePort}.`;
    let noteMessage =
      preferredPort === normalizedConfiguredPort
        ? `Saved default port: ${normalizedConfiguredPort}.`
        : `Saved default port: ${normalizedConfiguredPort}. Current preference in use: ${preferredPort}.`;

    if (overriddenByEnv) {
      statusMessage +=
        " This session was started with an environment override.";
      noteMessage +=
        " Clear TTS_STORY_PORT/PORT to use the saved setting next time.";
    } else if (fallbackActive) {
      statusMessage +=
        " The saved port was busy, so this session auto-switched to a fallback port.";
      noteMessage +=
        " Save a different port here if you want future launches to avoid that detour.";
    } else if (activePort !== normalizedConfiguredPort) {
      statusMessage += " Save and restart to apply the new port.";
    } else {
      noteMessage += " Changes apply on the next launch.";
    }

    statusEl.textContent = statusMessage;
    noteEl.textContent = noteMessage;
  } catch (error) {
    console.error("Error loading server port status:", error);
    statusEl.textContent = `Saved default port: ${normalizedConfiguredPort}.`;
    noteEl.textContent = "Port changes apply on the next launch.";
  }
}
...
function updateLLMSettingsUI(provider = "gemini") {
  ...
  if (geminiCredentials)
    geminiCredentials.style.display = isLocal ? "none" : "";
  if (geminiModelGroup) geminiModelGroup.style.display = isLocal ? "none" : "";
  if (geminiModelsActions)
    geminiModelsActions.style.display = isLocal ? "none" : "";
...
async function fetchLocalLlmModels(buttonEl) {
  ...
  try {
    ...
    if (!models.length) {
      throw new Error(
        "No local models were returned. Verify the server is running.",
      );
    }
...
function updateLocalProviderDefaults(nextProvider) {
  ...
  if (
    !currentValue ||
    Object.values(LOCAL_LLM_BASE_URLS).includes(currentValue)
  ) {
...
async function persistGeminiPresets(feedbackMessage) {
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("geminiPresets:changed", {
        detail: {
          presets: payload,
        },
      }),
    ...
    updateGeminiPresetHint(
      "Preset list updated locally but failed to save. Try again.",
      "warning",
    );
  } finally {
...
function setGeminiPresetState(presets = []) {
  ...
  updateGeminiPresetHint(
    "Fill both fields to create a new preset, or select Edit on an existing preset.",
  );
}
...
async function handleGeminiPresetSave() {
  ...
  if (geminiPresetState.editingId) {
    const idx = geminiPresetState.list.findIndex(
      (preset) => preset.id === geminiPresetState.editingId,
    );
    if (idx !== -1) {
...
async function handleGeminiPresetListClick(event) {
  ...
  if (action === "edit") {
    const preset = geminiPresetState.list.find(
    ...
    updateGeminiPresetHint(
      `Editing preset "${preset.title}". Save to apply changes or Clear to cancel.`,
      "info",
    );
  } else if (action === "delete") {
    const index = geminiPresetState.list.findIndex(
      (entry) => entry.id === presetId,
    );
    if (index === -1) return;
...
function toggleEngineSettingsSections(engineName) {
  ...
  if (targetTab) {
    const tabBtn = document.querySelector(
      `.engine-tab-btn[data-engine-tab="${targetTab}"]`,
    );
    if (tabBtn) {
      tabBtn.click();
    }
...
function setupSettingsListeners() {
  ...
  const fetchGeminiModelsBtn = document.getElementById(
    "fetch-gemini-models-btn",
  );
  if (fetchGeminiModelsBtn) {
    fetchGeminiModelsBtn.addEventListener("click", () =>
      fetchGeminiModels(fetchGeminiModelsBtn),
    );
  }
  const fetchLocalModelsBtn = document.getElementById(
    "fetch-local-llm-models-btn",
  );
  if (fetchLocalModelsBtn) {
    fetchLocalModelsBtn.addEventListener("click", () =>
      fetchLocalLlmModels(fetchLocalModelsBtn),
    );
  }
...
function setupSettingsAccordion() {
  const headers = document.querySelectorAll(
    '.settings-group-header[data-toggle="settings-group"]',
  );
...
function setupEngineTabSwitching() {
  const tabButtons = document.querySelectorAll(
    ".engine-tab-btn[data-engine-tab]",
  );
  ...
  try {
    ...
    if (statusEl) {
      statusEl.textContent =
        error.message ||
        "Unable to fetch models. Check the console for details.";
    }
  } finally {
    if (buttonEl) {
      buttonEl.disabled = false;
...
async function loadSettings() {
  try {
    ...
    if (data.success) {
      ...
      await refreshServerPortStatus(data.settings?.server_port);
    }
  } catch (error) {
...
function applySettings(settings) {
  ...
  setElementValue("settings-server-port", settings.server_port ?? 5000, 5000);
  ...
  setCheckboxValue(
    ...
    settings.group_chunks_by_speaker ?? false,
    false,
  );
  ...
  if (geminiModelSelect && savedGeminiModel) {
    const hasOption = Array.from(geminiModelSelect.options).some(
      (option) => option.value === savedGeminiModel,
    );
    if (!hasOption) {
  ...
  setElementValue(
    "gemini-speaker-profile-prompt",
    settings.gemini_speaker_profile_prompt || "",
  );
  ...
  setElementValue(
    "llm-local-provider",
    settings.llm_local_provider || "lmstudio",
    "lmstudio",
  );
  setElementValue(
    "llm-local-base-url",
    settings.llm_local_base_url || LOCAL_LLM_BASE_URLS.lmstudio,
    LOCAL_LLM_BASE_URLS.lmstudio,
  );
  ...
  setElementValue(
    "llm-local-temperature",
    settings.llm_local_temperature ?? 0.2,
    0.2,
  );
  ...
  setElementValue(
    "llm-local-repeat-penalty",
    settings.llm_local_repeat_penalty ?? 1.0,
    1.0,
  );
  setElementValue(
    "llm-local-max-tokens",
    settings.llm_local_max_tokens ?? 0,
    0,
  );
  setCheckboxValue(
    "llm-local-disable-reasoning",
    settings.llm_local_disable_reasoning ?? false,
    false,
  );
  setElementValue(
    "llm-gemini-chunk-size",
    settings.llm_gemini_chunk_size ?? 500,
    500,
  );
  setElementValue(
    "llm-local-chunk-size",
    settings.llm_local_chunk_size ?? 500,
    500,
  );
  setCheckboxValue(
    "llm-gemini-chunk-chapters",
    settings.llm_gemini_chunk_chapters ?? true,
    true,
  );
  setCheckboxValue(
    "llm-local-chunk-chapters",
    settings.llm_local_chunk_chapters ?? true,
    true,
  );
  ...
  const localDeviceInput = document.getElementById(
    "chatterbox-turbo-local-device",
  );
  if (localDeviceInput) {
    localDeviceInput.value = settings.chatterbox_turbo_local_device || "auto";
  }
  const localPromptInput = document.getElementById(
    "chatterbox-turbo-local-prompt",
  );
  if (localPromptInput) {
    localPromptInput.value =
      settings.chatterbox_turbo_local_default_prompt || "";
  }
  const localTemp = document.getElementById(
    "chatterbox-turbo-local-temperature",
  );
  if (localTemp) {
  ...
  const localRepPenalty = document.getElementById(
    "chatterbox-turbo-local-rep-penalty",
  );
  if (localRepPenalty) {
    localRepPenalty.value =
  ...
  const localExaggeration = document.getElementById(
    "chatterbox-turbo-local-exaggeration",
  );
  if (localExaggeration) {
    localExaggeration.value =
  ...
  const localPromptNorm = document.getElementById(
    "chatterbox-turbo-local-prompt-norm",
  );
  if (localPromptNorm) {
    localPromptNorm.checked =
      settings.chatterbox_turbo_local_prompt_norm_loudness !== false;
  }
  const localChunkSize = document.getElementById(
    "chatterbox-turbo-local-chunk-size",
  );
  if (localChunkSize) {
  ...
  if (qwen3Model) {
    qwen3Model.value =
      settings.qwen3_custom_model_id || "Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice";
  }
  ...
  if (qwen3Attn) {
    qwen3Attn.value =
      settings.qwen3_custom_attn_implementation || "flash_attention_2";
  }
  ...
  if (qwen3CloneModel) {
    qwen3CloneModel.value =
      settings.qwen3_clone_model_id || "Qwen/Qwen3-TTS-12Hz-1.7B-Base";
  }
  ...
  if (qwen3CloneAttn) {
    qwen3CloneAttn.value =
      settings.qwen3_clone_attn_implementation || "flash_attention_2";
  }
  ...
  const qwen3ClonePromptText = document.getElementById(
    "qwen3-clone-prompt-text",
  );
  ...
  if (kittenModelId) {
    kittenModelId.value =
      settings.kitten_tts_model_id || "KittenML/kitten-tts-mini-0.8";
  }
  ...
  const indexDefaultPrompt = document.getElementById(
    "index-tts-default-prompt",
  );
  ...
  const indexUseTorchCompile = document.getElementById(
    "index-tts-use-torch-compile",
  );
  if (indexUseTorchCompile) {
    indexUseTorchCompile.checked =
      settings.index_tts_use_torch_compile === true;
  }
  ...
  const indexDiffusionSteps = document.getElementById(
    "index-tts-diffusion-steps",
  );
  ...
  const indexRepetitionPenalty = document.getElementById(
    "index-tts-repetition-penalty",
  );
  if (indexRepetitionPenalty) {
    indexRepetitionPenalty.value =
      settings.index_tts_repetition_penalty ?? 10.0;
  }
  ...
  const indexMaxTextTokens = document.getElementById(
    "index-tts-max-text-tokens-per-segment",
  );
  if (indexMaxTextTokens) {
    indexMaxTextTokens.value =
      settings.index_tts_max_text_tokens_per_segment ?? 120;
  }

  // Chatterbox Replicate settings (uses shared replicate_api_key)
  const turboModelInput = document.getElementById(
    "chatterbox-turbo-replicate-model",
  );
  if (turboModelInput) {
    turboModelInput.value = settings.chatterbox_turbo_replicate_model || "";
  }
  const turboVoiceInput = document.getElementById(
    "chatterbox-turbo-replicate-voice",
  );
  if (turboVoiceInput) {
    turboVoiceInput.value = settings.chatterbox_turbo_replicate_voice || "";
  }
  const turboTempInput = document.getElementById(
    "chatterbox-turbo-replicate-temperature",
  );
  if (turboTempInput) {
    turboTempInput.value =
      settings.chatterbox_turbo_replicate_temperature ?? 0.8;
  }
  const turboTopPInput = document.getElementById(
    "chatterbox-turbo-replicate-top-p",
  );
  if (turboTopPInput) {
    turboTopPInput.value = settings.chatterbox_turbo_replicate_top_p ?? 0.95;
  }
  const turboTopKInput = document.getElementById(
    "chatterbox-turbo-replicate-top-k",
  );
  if (turboTopKInput) {
    turboTopKInput.value = settings.chatterbox_turbo_replicate_top_k ?? 1000;
  }
  const turboRepPenaltyInput = document.getElementById(
    "chatterbox-turbo-replicate-rep-penalty",
  );
  if (turboRepPenaltyInput) {
    turboRepPenaltyInput.value =
      settings.chatterbox_turbo_replicate_repetition_penalty ?? 1.2;
  }
  const turboSeedInput = document.getElementById(
    "chatterbox-turbo-replicate-seed",
  );
  if (turboSeedInput) {
...
async function saveSettings() {
  const defaultFormatSelect = document.getElementById("settings-output-format");
  const defaultBitrateSelect = document.getElementById(
    "settings-output-bitrate",
  );
  const serverPortInput = document.getElementById("settings-server-port");
  const defaultFormat = defaultFormatSelect ? defaultFormatSelect.value : "mp3";
  const defaultBitrate = defaultBitrateSelect
    ? parseInt(defaultBitrateSelect.value, 10) || 128
    : 128;
  const serverPort = normalizeServerPort(serverPortInput?.value, 5000);
  ...
  const kokoroReplicateKeyEl = document.getElementById(
    "kokoro-replicate-api-key",
  );
  const settings = {
    ...
    kokoro_chunk_size:
      parseInt(document.getElementById("kokoro-chunk-size")?.value, 10) || 500,
    ...
    parallel_chunks: Math.min(
      8,
      Math.max(
        1,
        parseInt(document.getElementById("parallel-chunks")?.value, 10) || 3,
      ),
    ),
    group_chunks_by_speaker:
      document.getElementById("group-chunks-by-speaker")?.checked ?? false,
    cleanup_vram_after_job:
      document.getElementById("cleanup-vram-after-job")?.checked ?? false,
    ...
    gemini_speaker_profile_prompt:
      document.getElementById("gemini-speaker-profile-prompt")?.value || "",
    gemini_prompt_presets: geminiPresetState.list.map((preset) => ({
      ...preset,
    })),
    llm_provider: document.getElementById("llm-provider")?.value || "gemini",
    llm_local_provider:
      document.getElementById("llm-local-provider")?.value || "lmstudio",
    llm_local_base_url:
      document.getElementById("llm-local-base-url")?.value ||
      LOCAL_LLM_BASE_URLS.lmstudio,
    llm_local_model: document.getElementById("llm-local-model")?.value || "",
    llm_local_api_key:
      document.getElementById("llm-local-api-key")?.value || "",
    llm_local_timeout:
      parseInt(document.getElementById("llm-local-timeout")?.value, 10) || 120,
    llm_local_temperature:
      parseFloat(document.getElementById("llm-local-temperature")?.value) ||
      0.2,
    llm_local_top_p:
      parseFloat(document.getElementById("llm-local-top-p")?.value) || 1.0,
    llm_local_top_k:
      parseInt(document.getElementById("llm-local-top-k")?.value, 10) || 0,
    llm_local_repeat_penalty:
      parseFloat(document.getElementById("llm-local-repeat-penalty")?.value) ||
      1.0,
    llm_local_max_tokens:
      parseInt(document.getElementById("llm-local-max-tokens")?.value, 10) || 0,
    llm_local_disable_reasoning:
      document.getElementById("llm-local-disable-reasoning")?.checked ?? false,
    llm_gemini_chunk_size: Math.max(
      50,
      parseInt(document.getElementById("llm-gemini-chunk-size")?.value, 10) ||
        500,
    ),
    llm_local_chunk_size: Math.max(
      50,
      parseInt(document.getElementById("llm-local-chunk-size")?.value, 10) ||
        500,
    ),
    llm_gemini_chunk_chapters:
      document.getElementById("llm-gemini-chunk-chapters")?.checked ?? true,
    llm_local_chunk_chapters:
      document.getElementById("llm-local-chunk-chapters")?.checked ?? true,
    tts_engine: document.getElementById("settings-tts-engine").value,
    chatterbox_turbo_local_device: document.getElementById(
      "chatterbox-turbo-local-device",
    ).value,
    chatterbox_turbo_local_default_prompt: document.getElementById(
      "chatterbox-turbo-local-prompt",
    ).value,
    chatterbox_turbo_local_temperature:
      parseFloat(
        document.getElementById("chatterbox-turbo-local-temperature").value,
      ) || 0.8,
    chatterbox_turbo_local_top_p:
      parseFloat(
        document.getElementById("chatterbox-turbo-local-top-p").value,
      ) || 0.95,
    chatterbox_turbo_local_top_k:
      parseInt(
        document.getElementById("chatterbox-turbo-local-top-k").value,
        10,
      ) || 1000,
    chatterbox_turbo_local_repetition_penalty:
      parseFloat(
        document.getElementById("chatterbox-turbo-local-rep-penalty").value,
      ) || 1.2,
    chatterbox_turbo_local_cfg_weight:
      parseFloat(
        document.getElementById("chatterbox-turbo-local-cfg-weight").value,
      ) || 0,
    chatterbox_turbo_local_exaggeration:
      parseFloat(
        document.getElementById("chatterbox-turbo-local-exaggeration").value,
      ) || 0,
    chatterbox_turbo_local_norm_loudness: document.getElementById(
      "chatterbox-turbo-local-norm",
    ).checked,
    chatterbox_turbo_local_prompt_norm_loudness: document.getElementById(
      "chatterbox-turbo-local-prompt-norm",
    ).checked,
    chatterbox_turbo_local_chunk_size:
      parseInt(
        document.getElementById("chatterbox-turbo-local-chunk-size").value,
        10,
      ) || 450,
    voxcpm_local_model_id: document.getElementById("voxcpm-local-model-id")
      .value,
    voxcpm_chunk_size:
      parseInt(document.getElementById("voxcpm-chunk-size")?.value, 10) || 550,
    voxcpm_local_device: document.getElementById("voxcpm-local-device").value,
    voxcpm_local_default_prompt: document.getElementById("voxcpm-local-prompt")
      .value,
    voxcpm_local_default_prompt_text: document.getElementById(
      "voxcpm-local-prompt-text",
    ).value,
    voxcpm_local_cfg_value:
      parseFloat(document.getElementById("voxcpm-local-cfg").value) || 2.0,
    voxcpm_local_inference_timesteps:
      parseInt(document.getElementById("voxcpm-local-steps").value, 10) || 10,
    voxcpm_local_normalize: document.getElementById("voxcpm-local-normalize")
      .checked,
    voxcpm_local_denoise: document.getElementById("voxcpm-local-denoise")
      .checked,
    qwen3_custom_model_id: document.getElementById("qwen3-custom-model-id")
      .value,
    qwen3_chunk_size:
      parseInt(document.getElementById("qwen3-chunk-size")?.value, 10) || 500,
    ...
    qwen3_custom_attn_implementation:
      document.getElementById("qwen3-custom-attn").value,
    qwen3_custom_default_language: document.getElementById(
      "qwen3-custom-language",
    ).value,
    qwen3_custom_default_instruct: document.getElementById(
      "qwen3-custom-instruct",
    ).value,
    ...
    qwen3_clone_attn_implementation:
      document.getElementById("qwen3-clone-attn").value,
    qwen3_clone_default_language: document.getElementById(
      "qwen3-clone-language",
    ).value,
    qwen3_clone_default_prompt:
      document.getElementById("qwen3-clone-prompt").value,
    qwen3_clone_default_prompt_text: document.getElementById(
      "qwen3-clone-prompt-text",
    ).value,
    pocket_tts_model_variant:
      document.getElementById("pocket-tts-model-variant")?.value || "b6369a24",
    pocket_tts_chunk_size:
      parseInt(document.getElementById("pocket-tts-chunk-size")?.value, 10) ||
      450,
    pocket_tts_temp:
      parseFloat(document.getElementById("pocket-tts-temp")?.value) || 0.7,
    pocket_tts_lsd_decode_steps:
      parseInt(document.getElementById("pocket-tts-steps")?.value, 10) || 1,
    pocket_tts_noise_clamp: (() => {
      const raw = document
        .getElementById("pocket-tts-noise-clamp")
        ?.value?.trim();
      if (!raw) return null;
    ...
    pocket_tts_eos_threshold:
      parseFloat(document.getElementById("pocket-tts-eos")?.value) || -4.0,
    pocket_tts_default_prompt:
      document.getElementById("pocket-tts-default-prompt")?.value || "",
    pocket_tts_prompt_truncate:
      document.getElementById("pocket-tts-prompt-truncate")?.checked ?? false,
    pocket_tts_num_threads: (() => {
      const raw = document
        .getElementById("pocket-tts-num-threads")
        ?.value?.trim();
      if (!raw) return null;
    ...
    pocket_tts_interop_threads: (() => {
      const raw = document
        .getElementById("pocket-tts-interop-threads")
        ?.value?.trim();
      if (!raw) return null;
      const parsed = parseInt(raw, 10);
      return Number.isFinite(parsed) ? parsed : null;
    ...
    kitten_tts_model_id:
      document.getElementById("kitten-tts-model-id")?.value ||
      "KittenML/kitten-tts-mini-0.8",
    kitten_tts_default_voice:
      document.getElementById("kitten-tts-default-voice")?.value || "Jasper",
    kitten_tts_chunk_size:
      parseInt(document.getElementById("kitten-tts-chunk-size")?.value, 10) ||
      300,
    index_tts_model_version:
      document.getElementById("index-tts-model-version")?.value || "IndexTTS-2",
    index_tts_chunk_size:
      parseInt(document.getElementById("index-tts-chunk-size")?.value, 10) ||
      400,
    index_tts_device:
      document.getElementById("index-tts-device")?.value || "auto",
    index_tts_default_prompt:
      document.getElementById("index-tts-default-prompt")?.value || "",
    index_tts_use_fp16:
      document.getElementById("index-tts-use-fp16")?.checked ?? true,
    index_tts_use_deepspeed:
      document.getElementById("index-tts-use-deepspeed")?.checked ?? false,
    index_tts_use_torch_compile:
      document.getElementById("index-tts-use-torch-compile")?.checked ?? false,
    index_tts_use_accel:
      document.getElementById("index-tts-use-accel")?.checked ?? false,
    index_tts_num_beams:
      parseInt(document.getElementById("index-tts-num-beams")?.value, 10) || 1,
    index_tts_diffusion_steps:
      parseInt(
        document.getElementById("index-tts-diffusion-steps")?.value,
        10,
      ) || 25,
    index_tts_temperature:
      parseFloat(document.getElementById("index-tts-temperature")?.value) ||
      0.8,
    index_tts_top_p:
      parseFloat(document.getElementById("index-tts-top-p")?.value) || 0.8,
    index_tts_top_k:
      parseInt(document.getElementById("index-tts-top-k")?.value, 10) || 30,
    index_tts_repetition_penalty:
      parseFloat(
        document.getElementById("index-tts-repetition-penalty")?.value,
      ) || 10.0,
    index_tts_max_mel_tokens:
      parseInt(
        document.getElementById("index-tts-max-mel-tokens")?.value,
        10,
      ) || 1500,
    index_tts_max_text_tokens_per_segment:
      parseInt(
        document.getElementById("index-tts-max-text-tokens-per-segment")?.value,
        10,
      ) || 120,
    chatterbox_turbo_replicate_model: document.getElementById(
      "chatterbox-turbo-replicate-model",
    ).value,
    chatterbox_turbo_replicate_voice: document.getElementById(
      "chatterbox-turbo-replicate-voice",
    ).value,
    chatterbox_turbo_replicate_temperature:
      parseFloat(
        document.getElementById("chatterbox-turbo-replicate-temperature").value,
      ) || 0.8,
    chatterbox_turbo_replicate_top_p:
      parseFloat(
        document.getElementById("chatterbox-turbo-replicate-top-p").value,
      ) || 0.95,
    chatterbox_turbo_replicate_top_k:
      parseInt(
        document.getElementById("chatterbox-turbo-replicate-top-k").value,
        10,
      ) || 1000,
    chatterbox_turbo_replicate_repetition_penalty:
      parseFloat(
        document.getElementById("chatterbox-turbo-replicate-rep-penalty").value,
      ) || 1.2,
    chatterbox_turbo_replicate_seed: (() => {
      const raw = document
        .getElementById("chatterbox-turbo-replicate-seed")
        .value.trim();
      if (!raw) return null;
      const parsed = parseInt(raw, 10);
      return Number.isFinite(parsed) ? parsed : null;
  ...
  try {
    ...
    if (data.success) {
      await refreshServerPortStatus(settings.server_port);
      alert(
        "Settings saved successfully! Port changes apply after restarting TTS-Story.",
      );
...
async function resetSettings() {
  ...
  try {
    ...
    if (data.success) {
      ...
      await refreshServerPortStatus(defaults.server_port);
